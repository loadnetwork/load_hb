...
pub fn list_objects(
    ...
    rt.block_on(async {
        let client = crate::s3::create_s3_client(&endpoint, &access_key_id, &secret_access_key, &region, Some(true)).await;

        match crate::s3::list_objects(&client, &bucket, &prefix, &delimiter, &marker, &max_keys).await {
            Ok(output) => {
                ...
                let encoding_type = output.encoding_type().map(|et| et.as_str()).unwrap_or("").to_string();
                let request_charged = output.request_charged().map(|rc| rc.as_str()).unwrap_or("").to_string();
                ...
                result.insert("encoding_type".to_string(), encoding_type.as_bytes().to_vec());
                result.insert("request_charged".to_string(), request_charged.as_bytes().to_vec());
                ...
                if !contents.is_empty() {
                    for (i, object) in contents.iter().enumerate() {
                        ...
                        let storage_class = object.storage_class().map(|sc| sc.as_str()).unwrap_or("");
                        let owner_display_name = object.owner().and_then(|o| o.display_name()).unwrap_or("");
                        ...
                        result.insert(format!("object_{}_last_modified", i), last_modified.as_bytes().to_vec());
                        result.insert(format!("object_{}_etag", i), etag.as_bytes().to_vec());
                        result.insert(format!("object_{}_size", i), size.as_bytes().to_vec());
                        result.insert(format!("object_{}_storage_class", i), storage_class.as_bytes().to_vec());
                        result.insert(format!("object_{}_owner_display_name", i), owner_display_name.as_bytes().to_vec());
                        result.insert(format!("object_{}_owner_id", i), owner_id.as_bytes().to_vec());
                    }
                    result.insert("object_count".to_string(), contents.len().to_string().as_bytes().to_vec());
                } else {
                ...
                if !common_prefixes.is_empty() {
                    ...
                    result.insert("common_prefix_count".to_string(), common_prefixes.len().to_string().as_bytes().to_vec());
                } else {
