...
pub(crate) fn parse_ans104_header(header: &[u8]) -> Result<(String, usize), String> {
    ...
    let sig_type = cursor.read_u16::<LittleEndian>().map_err(|_| "read signature_type failed")?;
    if sig_type != 1 { return Err("expected Arweave signer type 1".into()); }
    ...
    let _tag_count = cursor.read_u64::<LittleEndian>().map_err(|_| "read tag_count failed")?;
    let tag_bytes_len = cursor.read_u64::<LittleEndian>().map_err(|_| "read tag_bytes_len failed")? as usize;
    let tag_start = cursor.position() as usize;
    let tag_end = tag_start + tag_bytes_len;
    if tag_end > header.len() { return Err("header too small for tags".into()); }    
